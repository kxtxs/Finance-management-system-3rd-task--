...
// Include the FinanceManager header where Wallet and Transaction are defined
...
// iomanip – for formatting (e.g., setprecision)
// fstream – for file operations
// algorithm – for sort, transform, and other algorithms
// map – for summing amounts by category

using namespace std;
// Allows using cout, string, etc. without std::

// ======== Time functions ========
time_t FinanceManager::nowTime() {
    return time(nullptr);
}
// Returns the current time in seconds since 1970-01-01 (UNIX time)

time_t FinanceManager::periodStartDays(int days) {
    if (days <= 0) return 0;
    // If days <= 0, return 0 meaning "all time"
    time_t now = nowTime();
    // Get current time
    return now - static_cast<time_t>(days) * 24 * 60 * 60;
    // Subtract the requested number of days in seconds
}

string FinanceManager::formatTime(time_t t) {
    if (t == 0) return string("-");
    // If t==0, return a dash (no date)
    tm local_tm;
#if defined(_MSC_VER)
...
#endif
    // Convert time_t to local time in a tm structure
    char buf[64];
...
bool FinanceManager::addWallet(const string& name, WalletType type, double creditLimit) {
    ...
    // Add a new Wallet to the vector
    return true;
}
...
// Return all wallets (const access)
...
void FinanceManager::showAllTransactions(int days) const {
    ...
    // Determine period start (0 = all time)
    bool any = false;  // Whether any transactions exist
...
vector<pair<string, double>> FinanceManager::topCategories(int days, int topN) const {
    ...
    // Map to accumulate sums by category
...
// ======== Print top categories to console ========
